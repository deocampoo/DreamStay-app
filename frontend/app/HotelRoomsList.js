...
export default function HotelRoomsList({ hotelName, checkin, checkout, open, onClose }) {
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((data) => {
        ...
        else setError(data.error || "No se pudieron cargar las habitaciones");
        setLoading(false);
      })
      .catch(() => {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.25)', zIndex: 1100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 32, minWidth: 420, maxWidth: 600, boxShadow: '0 4px 24px #0002', position: 'relative' }}>
        ...
        <div style={{ fontWeight: 600, fontSize: 22, marginBottom: 18 }}>Habitaciones de {hotelName}</div>
        {loading && <div>Cargando habitaciones...</div>}
        {error && <div style={{ color: '#DC2626', marginBottom: 12 }}>{error}</div>}
        {!loading && !error && rooms.length === 0 && <div>No hay habitaciones disponibles para las fechas seleccionadas.</div>}
        {!loading && !error && rooms.length > 0 && (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15 }}>
            <thead>
              <tr style={{ background: '#f1f5f9' }}>
                <th style={{ padding: 8, borderRadius: 6, textAlign: 'left' }}>Nombre</th>
                <th style={{ padding: 8, borderRadius: 6, textAlign: 'left' }}>Tipo</th>
                <th style={{ padding: 8, borderRadius: 6, textAlign: 'left' }}>Capacidad</th>
                <th style={{ padding: 8, borderRadius: 6, textAlign: 'left' }}>Precio/noche</th>
                <th style={{ padding: 8, borderRadius: 6, textAlign: 'left' }}>Estado</th>
              </tr>
            </thead>
            <tbody>
              {rooms.map((room, i) => (
                <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#f8fafc' }}>
                  ...
                  <td style={{ padding: 8 }}>${room.precio_por_noche}</td>
                  <td style={{ padding: 8 }}>
                    <span style={{ color: room.estado === 'disponible' ? '#16A34A' : '#DC2626', fontWeight: 600 }}>
